...
// Define proper types for the result object
...
interface FormatterResult {
  ...
  bedroomHouse?: BedroomData;
  bedroomUnit?: BedroomData;
  capitalAvg?: {
    house?: number;
    unit?: number;
  };
...
export function formatMarkdownReply(suburb: string, result: FormatterResult) {
  ...
  // Yield series
  const hs = (result.yieldSeries||[]).find((s: YieldSeries) => s.propertyType === "house");
  const us = (result.yieldSeries||[]).find((s: YieldSeries) => s.propertyType === "unit");
  const houseTrend = hs?.points?.filter((p: YieldPoint) => p.value != null).length
    ? `• 🏠 **House**: ${hs.points.map((p: YieldPoint) => `${p.year}: ${p.value?.toFixed(1)}%`).join(", ")}`
    : "";
  const unitTrend = us?.points?.filter((p: YieldPoint) => p.value != null).length
    ? `• 🏢 **Unit**: ${us.points.map((p: YieldPoint) => `${p.year}: ${p.value?.toFixed(1)}%`).join(", ")}`
    : "";
  ...
  const nearbyBlock = nb.length
    ? `\n🔎 **Nearby suburbs perspective**\n${nb.map((r: NearbyRow) => `• **${r.suburb}** → 🏠 House: ${r.house!=null?r.house.toFixed(1)+'%':'N/A'}, 🏢 Unit: ${r.unit!=null?r.unit.toFixed(1)+'%':'N/A'}`).join("\n")}`
    : "";
